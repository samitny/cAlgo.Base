...
namespace cAlgo.Robots
{
    ...
    public class TrailCut : Robot
    {
		...
		private WilliamsPercentRange williamsPercentRange;
        #endregion
		...
		private int signalStrategie()
		{
			...
			///////////////////////////////////////////////////////
			//if (williamsPercentRange.Result[williamsPercentRange.Result.Count - 1] < -95)
			//	foreach (Position position in Positions.FindAll(botName, Symbol, TradeType.Sell)) 
			//		ClosePosition(position);

			//if (williamsPercentRange.Result[williamsPercentRange.Result.Count - 1] > -5)
			//	foreach (Position position in Positions.FindAll(botName, Symbol, TradeType.Buy))
			//		ClosePosition(position);


			if ((williamsPercentRange.Result[williamsPercentRange.Result.Count - 2] < -80) && (williamsPercentRange.Result[williamsPercentRange.Result.Count - 1] > -80))
			{
				foreach (Position position in Positions.FindAll(botName, Symbol, TradeType.Sell)) 
					ClosePosition(position);
			...
			else if ((williamsPercentRange.Result[williamsPercentRange.Result.Count - 2] > -20) && (williamsPercentRange.Result[williamsPercentRange.Result.Count - 1] < -20))
			{
				foreach (Position position in Positions.FindAll(botName, Symbol, TradeType.Buy))
					ClosePosition(position);
